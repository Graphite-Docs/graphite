...
export default class GraphiteAdmin extends Component {
  ...
  componentDidMount() {
    ...
    getFile('automergeTest1.json', {decrypt: true})
      .then((fileContents) => {
        if(fileContents) {
          console.log("Found your file");
          console.log(JSON.parse(fileContents || '{}').team);
          this.setState({
      ...
      .then(() => {
        if(this.state.team === null || this.state.team.length < 1) {
          ...
          object = Automerge.change(object, file => {
            ...
            file.editorPermissions = this.state.editorPermissions;
            ...
            file.journoName = this.state.journoName;
            file.journoRoles = this.state.journoRoles;
            file.journoPermissions = this.state.journoPermissions;
            file.journoIntegrations = this.state.journoIntegrations;
          });
    ...
    getFile(getPublicKeyFromPrivate(loadUserData().appPrivateKey) + '.json', options)
     .then((fileContents) => {
       if(fileContents) {
         ...
         console.log(JSON.parse(decryptECIES(privateKey, JSON.parse(fileContents))).team);
         this.setState({
      ...
      if(team.length > count) {
        ...
        if(loadUserData().username !== user) {
          ...
          getFile(file, options)
            ...
            .then(() => {
              let finalObject = Automerge.merge(this.state.file2, this.state.file1);
