...
import { loadSingleVaultFile } from "../../../files/helpers/singleVaultFile";
import { handleVaultDrop } from "../../../files/helpers/newVaultFile";
    ...
    if (src) {
      if (src.includes("status/")) {
        ...
        for (t = 0; t < tweets.length; t++) {
          let id = document
            .getElementsByClassName("tweet")[t].getAttribute("id");
          window.twttr.widgets.createTweet(id, tweets[t], {
      ...
      setGlobal({ myTimeline: initialTimeline }, () => {
        new window.TL.Timeline("timeline-embed", initialTimeline);
      });
    }
  }, 1000);
}
...
class SlateEditor extends React.Component {
  ...
  onKeyDown = (event, editor, next) => {
    ...
    if (isBoldHotkey(event)) {
      ...
      (event.metaKey && event.keyCode === 90)
    ) {
      event.preventDefault();
  ...
  onBackspace = (event, editor, next) => {
    ...
    if (startBlock.type === "list-item") {
      editor.unwrapBlock("list").unwrapBlock("ordered");
        ...
        value.marks
          ...
          .forEach(mark => {
            editor
              ...
              .addMark({
                type: "color",
                data: { class: "color_" + color.hex.split("#")[1] }
              })
              .focus();
          });
      }
  ...
  onClickHighlight = color => {
    ...
    if (hasHighlight) {
      if (value.selection.isExpanded) {
        value.marks
          .filter(mark => mark.type === "highlight")
          .forEach(mark => {
            editor
              .removeMark(mark)
              .focus()
              .addMark({
                type: "highlight",
                data: { class: "background_" + color.hex.split("#")[1] }
              })
              .focus();
          });
      }
    ...
    if (type !== "list" && type !== "ordered" && type !== "check-list") {
      ...
      if (type === "check-list") {
        ...
        if (isCheckList && isType) {
          editor.setBlocks(DEFAULT_NODE).unwrapBlock("check-list");
        } else if (isCheckList) {
          editor.unwrapBlock("check-list").wrapBlock(type);
        }
      } else {
        const isList = this.hasBlock("list-item");
        const isType = value.blocks.some(block => {
        ...
        if (isList && isType) {
          editor
            ...
            .unwrapBlock(type === "list" ? "ordered" : "list")
            .unwrapBlock("ordered");
        } else if (isList) {
          console.log("blamo");
          editor
      ...
      if (type === "check-list") {
        editor
          .setBlocks("check-list-item", { data: { checked: false } })
          .wrapBlock(type);
      } else {
  ...
  onClickEmoji = code => {
    console.log(code);
    this.editor.insertText(code.native).focus();
  };
  ...
  handleTimelineEventUpdate = () => {
    this.close2();
    tline.handleAddNewTimelineEvent();
  };
  ...
  render() {
    ...
    const isTable =
      this.editor && this.editor.isSelectionInTable(this.global.content);
    return (
      <div>
        {!this.global.rtc &&
        ...
        {this.global.showCollab ? (
          <div className="authorship">
            <div>
              <h4 style={{ color: "#fff", marginLeft: "15px" }}>
                {this.props.uniqueID}...
              </h4>
            </div>
          </div>
  ...
  renderNode = (props, editor, next) => {
    ...
    switch (node.type) {
        ...
        return (
          ...
          >
            <label className="checkmarkContainer">
              ...
              />
              <span
                onClick={() => this.onCheckboxClick(node, !checked)}
                className="checkmark"
              />
            </label>
            <span style={{ marginLeft: "30px" }}>
              {checked ? (
                <strike style={{ color: "#cecece" }}>{children}</strike>
              ) : (
                <span>{children}</span>
              )}
            </span>
      ...
      case "emoji": {
        ...
        return (
          <span {...attributes} selected={isFocused} contentEditable={false}>
            {code}
          </span>
        );
      }
      case "align":
        ...
        return (
          <div {...attributes} className={align}>
            {children}
          </div>
        );
      case "check-list":
      ...
      case "embed": {
        ...
        if (src !== "none") {
          if (src.includes("youtube") || src.includes("vimeo")) {
            ...
            return <div className="tweet" id={src} />;
          }
        } else {
          return (
            <div>
              ...
              >
                <Modal.Content>
                  ...
                  <div>
                    ...
                    <h3>
                      Timeline Events ({myTimeline.events.length}){" "}
                      <Button
                        onClick={this.closeConfig2Show(true, true)}
                        secondary
                        style={{ borderRadius: "0" }}
                      >
                        New Event
                        ...
                        <Grid columns="two" divided>
                          <Grid.Row>
                            <Grid.Column>
                              <p>Media URL</p>
                              <Input
                                onChange={tline.handleTimelineEventMediaUrl}
                                placeholder="Enter a url to the media you want to use"
                              />{" "}
                              <br />
                              <p>Media Caption</p>
                              <Input
                                onChange={
                                  tline.handleTimelineEventMediaCaption
                                }
                                placeholder="Enter a caption for the media you want to use"
                              />{" "}
                              <br />
                              <p>Media Credit</p>
                              <Input
                                onChange={tline.handleTimelineEventMediaCredit}
                                placeholder="Give credit to the creator of the media"
                              />{" "}
                              <br />
                            </Grid.Column>
                            <Grid.Column>
                              ...
                              <Input
                                onChange={
                                  tline.handleTimelineEventTextHeadline
                                }
                                placeholder="Enter a headline"
                              />{" "}
                              <br />
                              <p>Event Sub-Text</p>
                              <Input
                                onChange={tline.handleTimelineEventTextText}
                                placeholder="Enter some sub-text"
                              />{" "}
                              <br />
                              <p>Event Month</p>
                              <Input
                                onChange={tline.handleTimelineEventStartMonth}
                                placeholder="Enter a headline"
                              />{" "}
                              <br />
                              <p>Event Day</p>
                              <Input
                                onChange={tline.handleTimelineEventStartDay}
                                placeholder="Enter some sub-text"
                              />{" "}
                              <br />
                              <p>Event Year</p>
                              <Input
                                onChange={tline.handleTimelineEventStartYear}
                                placeholder="Enter some sub-text"
                              />{" "}
                              <br />
                            </Grid.Column>
                            ...
                            <Table.Row key={a.unique_id}>
                              ...
                              <Table.Cell>
                                <button
                                  ...
                                  onClick={() =>
                                    tline.handleDeleteTimelineEvent(
                                      a.unique_id
                                    )
                                  }
                                >
  ...
  renderMark = (props, editor, next) => {
    ...
    switch (mark.type) {
      ...
      case "color":
        ...
        return (
          <span className={color} {...attributes}>
            {children}
          </span>
        );
      case "highlight":
        ...
        return (
          <span className={highlight} {...attributes}>
            {children}
          </span>
        );
      default:
