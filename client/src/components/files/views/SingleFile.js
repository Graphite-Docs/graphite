...
export default class SingleVaultFile extends Component {
  ...
  render() {
    const { publicVaultFile, singleFile, type, loading, pages, page, name, link, content, grid } = this.global;
    var thisStyle = {
      display: "none"
    };
    let sharedBy =loadUserData().username;
    let pagination = null;
    if (pages) {
    ...
    if(!loading) {
      return (
        ...
        <MainMenu style={{ borderRadius: "0", background: "#282828", color: "#fff" }}>
          ...
          <MainMenu.Item>
            <Modal trigger={<button className="link-button"><Icon style={{cursor: "pointer", color: "#fff"}} name='linkify' /></button>}>
                  ...
                  <div style={{marginTop: "15px"}}>
                    {
                      ...
                      <div><a id='shared-vault-link' href={window.location.origin + '/public/vault/' + sharedBy + '/' + window.location.href.split('vault/')[1].split('#')[0]} target='_blank' rel="noopener noreferrer" style={{wordWrap:"break-word"}}>{ window.location.origin + '/public/vault/' + sharedBy + '/' + window.location.href.split('vault/')[1]}</a><Icon onClick={this.copyLink} style={{marginLeft: "10px", cursor: "pointer"}} name='copy outline' /></div> :
                      <div className='hide' />
                    }
                  </div>
                </Modal.Description>
