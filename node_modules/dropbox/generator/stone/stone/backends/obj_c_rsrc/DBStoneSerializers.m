...
+ (NSString *)formatDateToken:(NSString *)token
{
    ...
    if ([token isEqualToString:@"%a"]) {  // Weekday as locale's abbreviated name.
        ...
        result = @"EEE";
                 ...
                 isEqualToString:@"%w"]) {  // Weekday as a decimal number, where 0 is Sunday and 6 is Saturday. 0, 1,
                                            // ..., 6
        result = @"ccccc";
    }
        ...
        result = @"DDD";
                 ...
                 isEqualToString:@"%U"]) {  // Week number of the year (Sunday as the first day of the week) as a zero
