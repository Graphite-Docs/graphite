...
describe('ColumnSorting', function () {
  ...
  var blackDownPointingTriangle = String.fromCharCode(9660); // ▼
  var blackUpPointingTriangle = String.fromCharCode(9650); // ▲
  ...
  beforeEach(function () {
    ...
    this.sortByClickOnColumnHeader = function (columnIndex) {
      var element = this.$container.data('handsontable').view.wt.wtTable.getColumnHeader(columnIndex).querySelector('span.columnSorting');
  ...
  it('should display sortIndicator properly after changing column order', function () {
    var modifyCol = function modifyCol(column) {
      if (column === 0) {
        return 1;
      } else if (column === 1) {
        return 0;
    ...
    updateSettings({ modifyCol: modifyCol });
  ...
  it('should render a correct number of TD elements after sorting', _asyncToGenerator(function* () {
    handsontable({
      data: [['1\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n'], ['2']],
      height: 100,
      colHeaders: true,
  ...
  it('should place empty strings, null and undefined values at proper position (stability of default comparing function)', function () {
    ...
    expect(getDataAtCol(1)).toEqual(['Frank Honest', 'Joan Well', 'Sid Strong', 'Chuck Jackson', 'Rob Norris', 'Eve Well',
    // empty cells below
    'Ted Right', 'Jane Neat', 'Meg Jansen', 'Sean Hara', 'Eve Branson']);

    getPlugin('columnSorting').sort(0, 'desc'); // DESC

    expect(getDataAtCol(1)).toEqual(['Eve Well', 'Rob Norris', 'Chuck Jackson', 'Sid Strong', 'Joan Well', 'Frank Honest',
    // empty cells below
    'Ted Right', 'Jane Neat', 'Meg Jansen', 'Sean Hara', 'Eve Branson']);
  });
  ...
  it('should place empty strings, null and undefined values at proper position when `sortEmptyCells` option is enabled ' + '(API call, data type: default)', function () {
    ...
    expect(getDataAtCol(1)).toEqual(['Ted Right', 'Jane Neat', 'Meg Jansen', 'Sean Hara', 'Eve Branson',
    // empty cells above
    'Frank Honest', 'Joan Well', 'Sid Strong', 'Chuck Jackson', 'Rob Norris']);

    getPlugin('columnSorting').sort(0, 'desc'); // DESC

    expect(getDataAtCol(1)).toEqual(['Rob Norris', 'Chuck Jackson', 'Sid Strong', 'Joan Well', 'Frank Honest',
    // empty cells below
    'Ted Right', 'Jane Neat', 'Meg Jansen', 'Sean Hara', 'Eve Branson']);
  });
  ...
  describe('data type: date', function () {
    it('should place empty strings, null and undefined values at proper position when `sortEmptyCells` ' + 'option is enabled and `column` property of `columnSorting` option is set', function () {
      ...
      expect(getDataAtCol(0)).toEqual(['Citroen1', 'Citroen2', 'Citroen3', 'Citroen4', 'Citroen5',
      // empty cells above
      ...
      expect(getDataAtCol(0)).toEqual(['Audi1', 'BMW1', 'Mercedes1', 'Mercedes2', 'Opel1',
      // empty cells below
  ...
  it('should properly sort numeric data', _asyncToGenerator(function* () {
    handsontable({
