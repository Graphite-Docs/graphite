...
describe('Formulas utils', function () {
  ...
  describe('toUpperCaseFormula', function () {
    it('should correctly upper case formula expression', function () {
      ...
      expect(toUpperCaseFormula('=COUNTifs(dates, ">="&date(e5, 1, 1), dates, "<="&DATE(E5, 12, 31))')).toBe('=COUNTIFS(DATES, ">="&DATE(E5, 1, 1), DATES, "<="&DATE(E5, 12, 31))');
      expect(toUpperCaseFormula('=SumIf(range, "text*", SUM_range)')).toBe('=SUMIF(RANGE, "text*", SUM_RANGE)');
      expect(toUpperCaseFormula('=SumIf(range, \'text*\', SUM_range)')).toBe('=SUMIF(RANGE, \'text*\', SUM_RANGE)');
    });
  });
